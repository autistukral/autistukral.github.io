...
document.querySelectorAll(".ownBtn").forEach((ownButton) =>
...
document.querySelectorAll(".skins_img").forEach((imgButton) =>
...
function scrollToNextDiv(event) {
  ...
  if (currentTargetIndex < targetDivs.length - 1 && !scrolling) {
    ...
    targetDivs[currentTargetIndex].scrollIntoView({
      behavior: "auto",
      block: "center",
    });
...
function startScrolling(direction) {
  ...
  }, 400); // Set the delay in milliseconds
}
...
rmenu_class.forEach((rmenu_js) =>
  rmenu_js.addEventListener(
    "contextmenu",
    function (event) {
      event.preventDefault();
